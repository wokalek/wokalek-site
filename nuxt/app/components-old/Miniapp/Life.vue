  ...
  <div class="w-full">
    <form v-show="view === 'form'" ref="form" class="flex flex-col items-center" @submit.prevent="submitForm" @input="checkFormValidity">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-16-8">
        ...
        <input
          ...
          min="1900-01-01"
          max="2200-01-01"
          class="lead input-date-icon-reset min-w-full min-h-1.5em sm:min-w-0 sm:min-h-0 px-8-4 bg-transparent text-center select-none focus-within:outline-none ring-1 ring-grey-6 dark:ring-grey-3 sm:ml-1em place-self-center sm:place-self-start"
          type="date"
        >
        ...
        <input
          ...
          min="18"
          max="150"
          class="lead input-number-buttons-reset min-w-3em bg-transparent text-center select-none focus-within:outline-none ring-1 ring-grey-6 dark:ring-grey-3 sm:ml-1em place-self-center sm:place-self-start"
          type="number"
        >
      </div>
      <button class="lead mt-32-32 select-none transition-colors" :class="{ 'text-grey-3 dark:text-grey-4': isFormButtonDisabled, 'hover:text-grey-3': !isFormButtonDisabled }" type="submit">
        Показать
      </button>
    </form>
    <div v-show="view === 'table'">
      <div class="flex md:grid md:grid-flow-col flex-col justify-between items-start gap-x-16-8 gap-y-8-4 select-none">
        <div class="flex flex-wrap gap-x-16-8 gap-y-8-4">
          <button class="group/button" @click="backToForm('date')">
            <span class="text-grey-3">Дата рождения </span>
            <span class="text-nowrap group-hover/button:text-grey-3 transition-colors">
              {{ new Date(birthDate).toLocaleDateString('ru-RU') }}
            </span>
          </button>
          <button class="group/button" @click="backToForm('age')">
            <span class="text-grey-3">Возраст смерти </span>
            <span class="text-nowrap group-hover/button:text-grey-3 transition-colors">
              {{ deathAge }}
            </span>
          </button>
        ...
        <button class="text-left group/button" type="button" @click="switchTableMode">
          <span class="text-grey-3">Просмотр по</span> <span class="group-hover/button:text-grey-3 transition-colors">{{ tableMode === 'years' ? 'годам жизни' : 'календарным годам' }}</span>
        </button>
      </div>
      ...
      <div class="flex mt-32-16 cursor-pointer select-none" @click="copy(`${lifeProgress}%`)">
        <div>
          <span class="text-grey-3">Прогресс жизни</span> <span class="text-nowrap">{{ lifeProgress }}%</span>
        </div>
      </div>
    </div>
  </div>
...
.dot
  @apply border-1-1 border-grey-4 w-full h-0 pt-[100%] box-content

  &.wasted
    @apply bg-grey-5 border-grey-5
</style>
